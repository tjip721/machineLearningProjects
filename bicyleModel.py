...
class robot:
    ...
    def sense(self): #do not change the name of this function
        ...
        for ii in range(len(landmarks)):
            bearing = atan2(landmarks[ii][0]-self.y, landmarks[ii][1]-self.x)
            bearing = (bearing-self.orientation + 2*pi) % (2*pi) 
            Z.append(bearing)
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if(beta < 0.001):
            x = x1 + d*cos(theta)
            y = y1 + d*sin(theta)
            self.set(x,y,(theta+beta)%(2*pi))
        else:
            R = d/beta
            cx = x1 - sin(theta)*R
            cy = y1 + cos(theta)*R
            x = cx + sin(beta+theta)*R
            y = cy - cos(beta+theta)*R
            self.set(x,y,(theta+beta)%(2*pi))
